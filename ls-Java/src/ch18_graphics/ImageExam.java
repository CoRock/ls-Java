...
public class ImageExam extends JFrame implements ActionListener {
	...
	public ImageExam() {
		...
		/**
		 * 절대경로를 사용할 경우
		 * curImg = Toolkit.getDefaultToolkit().getImage("D:\\ls-Java\\src\\ch18_graphics\\flower.jpg");
		 */
		
		// 패널에 버튼 6개를 모아서 north 영역에 배치
		p = new Panel();
		
		btnNormal = new JButton("정상 그림");
		p.add(btnNormal);
		// NOT Recommended: p.add(btnNormal = new Button("정상 그림"));
	...
	// 버튼을 클릭하면 자동으로 actionPerformed method가 호출
	@Override
